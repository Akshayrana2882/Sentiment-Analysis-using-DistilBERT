# Sentiment-Analysis-using-DistilBERT
# Sentiment Analysis using DistilBERT

Developed an end-to-end sentiment analysis system using DistilBERT to classify texts into three categories: Negative, Neutral, and Positive. The project involves data preprocessing, tokenization using the `distilbert-base-uncased` tokenizer, fine-tuning a transformer-based sequence classification model, and evaluating performance using accuracy, precision, recall, F1-score, and confusion matrix.

**Model:** DistilBERT (`distilbert-base-uncased`)
**Classes:** Negative, Neutral, Positive
**Accuracy:** 96.75%
**Weighted F1-Score:** 0.97

## Features

* Multi-class sentiment classification
* Confidence scores for all sentiment classes
* Interactive Streamlit web application
* Fine-tuned transformer model for domain-specific sentiment analysis

## Tech Stack

Python, PyTorch, Hugging Face Transformers, Hugging Face Datasets, Pandas, NumPy, Scikit-learn, Matplotlib, Seaborn, Streamlit

## Project Structure

Sentiment-Analysis/
├── notebook.ipynb
├── app.py
├── requirements.txt
├── README.md
└── sentiment_model/
```



